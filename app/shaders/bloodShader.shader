    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                float timeOffsetL = _PTime*0.75; //смещение начала удара левого предсердия
                // _RR *= _Arytmia;
                ...
                vertModification.xyz +=  structColor.g * v.normal * (0.001- max((cos((_Time.y-_PQ-_QRS-v.vertex.y*5) * speed) * (1+zR1*not_q1 + zR2*q1) - (1-_R+zR1*not_q1 + zR2*q1))*coef*0.55, 0));// _Amount * 0.01; //желудочки
